...
#define TRACE_CPU_CLOCK_HZ 75000000U    /* Hardcoding : TBD */
...
#define traceTASK_SWITCHED_IN(taskID, taskPriority) \
			prvTraceStoreEvent2(PSF_EVENT_TASK_ACTIVATE, (uint32_t)taskID+1, taskPriority); \
...
#define traceMOVED_TASK_TO_READY_STATE( taskID ) \
		prvTraceStoreEvent1(PSF_EVENT_TASK_READY, taskID+1);
#endif
...
#define traceTASK_CREATE(taskID, taskName, taskPriority) \
	{ \
		prvTraceSaveSymbol(taskID+1, taskName); \
		prvTraceSaveObjectData(taskID+1, taskPriority); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, taskName, taskID+1); \
				prvTraceStoreEvent2(PSF_EVENT_TASK_CREATE, (uint32_t)taskID+1, taskPriority); \
	}
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_CREATE(pxNewTCB) \
	if (pxNewTCB != NULL) \
...
#define traceALARM_CREATE(alarmID, alarmName, ticks) \
	{ \
		prvTraceSaveSymbol(alarmID+10, alarmName); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, alarmName, alarmID+10); \
		prvTraceStoreEvent2(PSF_EVENT_ALARM_CREATE, (uint32_t)alarmID+10, ticks); \
	}
#else
#define traceALARM_CREATE(alarmID, alarmName, ticks)  
#endif
...
#define traceRESOURCE_CREATE(resID, resName, junk) \
	{ \
		prvTraceSaveSymbol(resID+20, resName); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, resName, resID+20); \
		prvTraceStoreEvent2(PSF_EVENT_RESOURCE_CREATE, (uint32_t)resID+20, junk); \
	}
#else
    #define traceRESOURCE_CREATE(resID, resName, junk)
#endif
...
#define traceALARM_SET_REL_ALARM(alarmID, ticks, cycle) \
		prvTraceStoreEvent2(PSF_EVENT_SET_REL_ALARM, (uint32_t)alarmID+10, cycle);
#else
    #define traceALARM_SET_REL_ALARM(alarmID, ticks, cycle)
#endif
...
#define traceALARM_SET_ABS_ALARM(alarmID, ticks, cycle) \
		prvTraceStoreEvent3(PSF_EVENT_SET_ABS_ALARM, (uint32_t)alarmID+10, ticks, cycle);
#else
  #define traceALARM_SET_ABS_ALARM(alarmID, ticks, cycle)  
#endif
...
#define traceALARM_CANCEL_ALARM(alarmID) \
		prvTraceStoreEvent1(PSF_EVENT_CANCEL_ALARM, (uint32_t)alarmID+10);
#else
#define traceALARM_CANCEL_ALARM(alarmID)
#endif
...
#define traceALARM_GET_ALARM(alarmID, ticks) \
		prvTraceStoreEvent2(PSF_EVENT_GET_ALARM, (uint32_t)alarmID+10, ticks);
#else
#define traceALARM_GET_ALARM(alarmID, ticks)    
#endif
...
#define traceRESOURCE_GET_RESOURCE(resID, status) \
		prvTraceStoreEvent2(PSF_EVENT_GET_RESOURCE, (uint32_t)resID+20, status);
#else
#define traceRESOURCE_GET_RESOURCE(resID, status)    
#endif
...
#define traceRESOURCE_RELEASE_RESOURCE(resID, status) \
		prvTraceStoreEvent2(PSF_EVENT_RELEASE_RESOURCE, (uint32_t)resID+20, status);
#else
#define traceRESOURCE_RELEASE_RESOURCE(resID, status)
#endif
...
#define traceTASK_GET_EVENT(taskID, event) \
	prvTraceStoreEvent2(PSF_EVENT_GET_EVENT, (uint32_t)taskID+1, event);
...
#define traceTASK_CLEAR_EVENT(taskID, event) \
		prvTraceStoreEvent2(PSF_EVENT_CLEAR_EVENT, (uint32_t)taskID+1, event);
#else
#define traceTASK_CLEAR_EVENT(taskID, event)    
#endif
...
#define traceALARM_ACTION_ACTIVATE_TASK(alarmID, taskID) \
		prvTraceStoreEvent2(PSF_EVENT_ACTION_ACTIVATE_TASK, (uint32_t)alarmID+10, (uint32_t)taskID+1);
#else
#define traceALARM_ACTION_ACTIVATE_TASK(alarmID, taskID)    
#endif
...
#define traceTASK_WAIT_EVENT(taskID, event) \
			prvTraceStoreEvent2(PSF_EVENT_WAIT_EVENT, (uint32_t)taskID+1, event); 
#else
...
#define traceTASK_SET_EVENT(taskID, event) \
			prvTraceStoreEvent2(PSF_EVENT_SET_EVENT, (uint32_t)taskID+1, event);
#else
#define traceTASK_SET_EVENT(taskID, event)    
#endif    
...
#define traceTASK_SET_EVENT_FROM_ISR(taskID, event) \
		prvTraceStoreEvent1(PSF_EVENT_SET_EVENT_FROM_ISR, (uint32_t)taskID+1, event);
#else
#define traceTASK_SET_EVENT_FROM_ISR(taskID, event)    
#endif  

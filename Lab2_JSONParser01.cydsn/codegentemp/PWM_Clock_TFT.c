...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_Start
********************************************************************************
*
* Summary:
...
void PWM_Clock_TFT_Start(void) 
{
    ...
    PWM_Clock_TFT_CLKEN |= PWM_Clock_TFT_CLKEN_MASK;
	PWM_Clock_TFT_CLKSTBY |= PWM_Clock_TFT_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_Stop
********************************************************************************
*
* Summary:
...
void PWM_Clock_TFT_Stop(void) 
{
    ...
    PWM_Clock_TFT_CLKEN &= (uint8)(~PWM_Clock_TFT_CLKEN_MASK);
	PWM_Clock_TFT_CLKSTBY &= (uint8)(~PWM_Clock_TFT_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PWM_Clock_TFT_CLKEN & PWM_Clock_TFT_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PWM_Clock_TFT_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_Clock_TFT_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_Clock_TFT__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_Clock_TFT_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_Clock_TFT_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PWM_Clock_TFT_GetSourceRegister();
    uint16 oldDivider = PWM_Clock_TFT_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_Clock_TFT_CLKEN & PWM_Clock_TFT_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PWM_Clock_TFT_DIV_PTR, clkDivider);
                PWM_Clock_TFT_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PWM_Clock_TFT_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PWM_Clock_TFT_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_Clock_TFT_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PWM_Clock_TFT_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Clock_TFT_MOD_SRC |= modeBitMask & (uint8)PWM_Clock_TFT_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Clock_TFT_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_Clock_TFT_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_Clock_TFT_MOD_SRC & (uint8)(PWM_Clock_TFT_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PWM_Clock_TFT_GetDividerRegister();
    uint8 oldSrc = PWM_Clock_TFT_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_Clock_TFT_MOD_SRC |= CYCLK_SSS;
        PWM_Clock_TFT_MOD_SRC =
            (PWM_Clock_TFT_MOD_SRC & (uint8)(~PWM_Clock_TFT_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_Clock_TFT_MOD_SRC & PWM_Clock_TFT_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_Clock_TFT_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Clock_TFT_PHASE = clkPhase & PWM_Clock_TFT_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_TFT_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PWM_Clock_TFT_PHASE & PWM_Clock_TFT_PHASE_MASK;
}
